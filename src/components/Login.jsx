...
import plus from "../assets/image/3dicons.png";
...
const Login = () => {
  ...
  const { openLogin, setOpenWapper, setOpenLogin } = useContext(HandleContext);
  const [recaptchaVerifier, setRecaptchaVerifier] = useState("");
  const [confirmationResult, setConfirmationResult] = useState(null);
  const server = process.env.REACT_APP_API_URL;
  const { getMyProfile, fromOrder, setOpenOrder } = useContext(HandleContext);
  ...
  return (
    <div
      ...
      } z-[1000] fixed  duration-300 ease-in-out top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 px-5 py-7 bg-white rounded-xl w-[400px] h-auto flex flex-col items-start justify-start  gap-20`}
